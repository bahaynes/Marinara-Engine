  ...
  if (enabled("roll")) {
    const skillsList =
      args.activeSkills && args.activeSkills.length > 0
        ...
        : "";
    lines.push(
      `- [roll: character="character name" notation="1d20" skill="Skill Name" reason="action and success rule"] requests a real roll; use roll_dice with the same fields when available. Skill (or attribute) is optional; the engine automatically adds the assigned ability modifier from character stats, so do not add it yourself. Set the stakes first, stop after the command, and wait for the result before narrating the outcome. Never invent results or reroll an action.${skillsList}`,
    );
  }
