    ...
    if (distanceFromEnd < 2) {
      ...
      const discoveries =
        normalized.keyDiscoveries.length > 0
          ...
          : "";
      lines.push(`Session ${normalized.sessionNumber} summary:`, mediumSummary, ...(discoveries ? [discoveries] : []));
    } else {
      const oneLine = extractOneLineSessionRecap(normalized.summary);
      const keywords = extractSessionKeywords(normalized);
      lines.push(`- Session ${normalized.sessionNumber}: ${oneLine}${keywords ? ` [Topics: ${keywords}]` : ""}`);
    }
