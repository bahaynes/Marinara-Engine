...
import type { DndCombatant, DndCombatState, DndActionRequest, DndSpell } from "../dnd-combat/types.js";
import {
  abilityModifier,
  proficiencyBonus,
  primaryCastingStat,
...
import { getCombatantWeapon, getWeaponAttackModifier } from "../dnd-combat/weapons.js";
import type { DndGridCoord, DndTacticalMap, DndTacticalState, DndTacticalUnitState } from "./types.js";
import {
  getDistanceFt,
  getReachableTiles,
  getAoEAffectedTiles,
...
export function resolveTacticalCombatRound(
  ...
  if (playerAction) {
    ...
    if (actor && target && actor.hp > 0 && target.hp > 0) {
      if (playerAction.type === "spell" && playerAction.spellId) {
        ...
        const spell =
          SPELL_CATALOG.find((s) => s.id === (playerAction.cantripId || playerAction.spellId)) || SPELL_CATALOG[0]!;
        const res = executeSingleSpell(actor, target, spell);
        target.hp = Math.max(0, target.hp - res.damageDealt);
        stepLogs.push({
          actorId: actor.id,
  ...
  for (const ally of alliesToAct) {
    ...
    if (!usedAoE) {
      ...
      if (move.costFt > 0) {
        ...
        const opp = checkOpportunityAttack(ally.id, allyState.coord, move.targetCoord, getLivingEnemies(), units);
        if (opp?.provoked) {
      ...
      if (closestEnemy.hp > 0) {
        if (isCaster) {
          ...
          const chosenSpell =
            availableSpells[Math.floor(Math.random() * Math.min(3, availableSpells.length))] || SPELL_CATALOG[0]!;
  ...
  for (const enemy of livingEnemies) {
    ...
    if (move.costFt > 0) {
      ...
      const opp = checkOpportunityAttack(enemy.id, enemyState.coord, move.targetCoord, getLivingAllies(), units);
      if (opp?.provoked) {
