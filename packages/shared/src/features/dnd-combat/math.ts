...
export function resolveDndAction(
  state: DndCombatState,
  action: DndActionRequest,
  ...
  const target =
    allActors.find((c) => c.id === action.targetId) ||
    (actor.side === "party" ? enemies.find((e) => e.hp > 0) : party.find((p) => p.hp > 0));
  if (!target) {
  ...
  const strikeLogs = strikes.map((s) => {
    ...
    if (s.isHit) {
      const extraText =
        (s.sneakDamage ? ` (+${s.sneakDamage} Sneak)` : "") + (s.smiteDamage ? ` (+${s.smiteDamage} Smite)` : "");
