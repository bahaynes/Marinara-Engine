...
function mapToDndCombatant(c: Combatant, side: "party" | "enemy", index: number, cards: any[] = []): DndCombatant {
  ...
  const cardText = [card?.description, card?.personality, card?.system_prompt, card?.mes_example, card?.creator_notes]
    .filter(Boolean)
    .join(" ");
  ...
  if (unitClass === "Warlock" && side === "party") {
    const classMatch = cardText.match(
      /\b(wizard|warlock|cleric|rogue|paladin|sorcerer|bard|druid|barbarian|fighter|ranger|monk)\b/i,
    );
...
export function DndCombatUI({
  ...
  const initialParty = useMemo(
    () => party.map((p, i) => mapToDndCombatant(p, "party", i, gameCharacterCards)),
    [party, gameCharacterCards],
  );
  const initialEnemies = useMemo(
    () => enemies.map((e, i) => mapToDndCombatant(e, "enemy", i, gameCharacterCards)),
    [enemies, gameCharacterCards],
  );
    ...
    (actionReq: Partial<DndActionRequest>) => {
      ...
      if (actionReq.spellId) {
        ...
        if (
          ...
          (spell.isAoE ||
            spell.id === "fireball" ||
            spell.id === "synaptic_static" ||
            spell.id === "cone_of_cold" ||
  ...
  const updateCombatantStat = useCallback((id: string, field: string, value: any) => {
    setDndState((prev) => ({
      ...
      party: prev.party.map((c) => {
        ...
        if (field === "maxHp")
          return { ...c, maxHp: Math.max(1, Number(value) || 10), hp: Math.min(c.hp, Number(value) || 10) };
        if (field in c.stats) {
          return {
            ...c,
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-white/10 bg-black/50 px-3 sm:px-4 py-2 sm:py-2.5 pr-28 sm:pr-36">
        <div className="flex flex-wrap items-center gap-2 sm:gap-3">
          <div className="flex items-center gap-1.5 rounded-lg bg-amber-500/20 px-2 sm:px-2.5 py-1 text-xs font-bold text-amber-300">
            <Sword size={14} />
            <span>{localizeUi("ui.game.dndcombatui.dD55e")}</span>
          </div>
          <span className="text-xs font-semibold text-white/60">
            {localizeUi("ui.game.gamecombatui.round")} {dndState.round}
          </span>
          ...
          <div className="flex items-center gap-1 rounded-lg bg-black/60 p-0.5 border border-white/10">
            ...
            >
              <Map size={13} />
              <span>{localizeUi("ui.game.dndcombatui.battlemap")}</span>
            </button>
            <button
              type="button"
            ...
            >
              <LayoutGrid size={13} />
              <span>{localizeUi("ui.game.dndcombatui.cards")}</span>
            </button>
          </div>
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              title={localizeUi("ui.game.dndcombatui.restartBattle")}
            >
              <RotateCcw size={13} />
              <span className="hidden md:inline">{localizeUi("ui.game.tacticalcombatui.restart")}</span>
            </button>
            <button
              type="button"
              ...
              title={localizeUi("ui.game.dndcombatui.fleeBattle")}
            >
              <Flag size={13} />
              <span className="hidden md:inline">{localizeUi("ui.game.tacticalcombatui.flee")}</span>
            </button>
          </div>
        </div>
      </div>
      ...
      {viewMode === "tactical" ? (
        <div className="flex min-h-0 flex-1 flex-col p-2 sm:p-3 gap-2">
          ...
          <div className="flex items-center gap-2 overflow-x-auto no-scrollbar py-1 shrink-0 bg-black/30 p-2 rounded-xl border border-white/10">
            <span className="text-[0.6875rem] font-bold uppercase text-white/50 shrink-0">
              {localizeUi("ui.game.dndcombatui.actor")}
            </span>
            ...
            <span className="text-[0.6875rem] font-bold uppercase text-red-400/70 shrink-0">
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-3 sm:gap-4 overflow-y-auto p-3 sm:p-6 overscroll-contain">
          ...
          <div>
            <h3 className="mb-1.5 sm:mb-2 text-xs font-bold uppercase tracking-wider text-red-400/80">
              {localizeUi("ui.game.dndcombatui.enemiesTapToTarget")}
            </h3>
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-1 rounded-md bg-white/10 px-1.5 sm:px-2 py-0.5 text-[0.6875rem] font-bold text-amber-200">
                        <Shield size={11} />
                        <span>
                          {localizeUi("ui.game.dndcombatui.ac")} {enemy.ac}
                        </span>
                      </div>
                    </div>
                    ...
                    <div>
                      <div className="mb-0.5 sm:mb-1 flex justify-between text-[0.6875rem] sm:text-xs font-semibold">
                        <span className="text-red-300">{localizeUi("ui.game.gamecharactersheet.hp")}</span>
          ...
          <div>
            <div className="mb-1.5 sm:mb-2 flex items-center justify-between">
              <h3 className="text-xs font-bold uppercase tracking-wider text-cyan-400/80">
                {localizeUi("ui.game.dndcombatui.partyMembers")}
              </h3>
              <span className="text-[0.6875rem] text-white/50">
                {localizeUi("ui.game.dndcombatui.tapCardToSelectActorEditStats")}
              </span>
            </div>
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <div className="flex items-center gap-1.5">
                            <span className="text-xs sm:text-sm font-bold text-white group-hover:text-cyan-200">
                              {member.name}
                            </span>
                            {i === 0 && (
                              <span className="rounded bg-cyan-500/20 px-1 text-[0.5625rem] font-bold text-cyan-300">
                                {localizeUi("ui.game.dndcombatui.you")}
                              </span>
                            )}
                          </div>
                          <div className="text-[0.6875rem] text-cyan-300/70 font-semibold">
                            {localizeUi("ui.game.dndcombatui.lvl")} {member.level} {member.unitClass}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-1.5">
                        <div className="flex items-center gap-1 rounded-md bg-cyan-900/40 px-1.5 sm:px-2 py-0.5 text-[0.6875rem] font-bold text-cyan-200 border border-cyan-500/30">
                          <Shield size={11} />
                          <span>
                            {localizeUi("ui.game.dndcombatui.ac")} {member.ac}
                          </span>
                        </div>
                        <button
                          type="button"
                    ...
                    <div className="flex flex-wrap items-center gap-1 text-[0.5625rem] sm:text-[0.625rem] font-bold text-white/70">
                      <span className="rounded bg-black/40 px-1 py-0.5">
                        {localizeUi("ui.game.gamecharactersheet.str")} {member.stats.str}
                      </span>
                      <span className="rounded bg-black/40 px-1 py-0.5">
                        {localizeUi("ui.game.dndcombatui.dex")} {member.stats.dex}
                      </span>
                      <span className="rounded bg-black/40 px-1 py-0.5">
                        {localizeUi("ui.game.dndcombatui.con")} {member.stats.con}
                      </span>
                      <span className="rounded bg-black/40 px-1 py-0.5">
                        {localizeUi("ui.game.dndcombatui.int")} {member.stats.int}
                      </span>
                      <span className="rounded bg-black/40 px-1 py-0.5">
                        {localizeUi("ui.game.dndcombatui.wis")} {member.stats.wis}
                      </span>
                    ...
                    <div>
                      <div className="mb-0.5 sm:mb-1 flex justify-between text-[0.6875rem] sm:text-xs font-semibold">
                        <span className="text-cyan-300">{localizeUi("ui.game.gamecharactersheet.hp")}</span>
                        <span className="text-white/70">
                          {member.hp} / {member.maxHp}
                        </span>
                      </div>
          ...
          <div className="flex-1 rounded-xl border border-white/10 bg-black/40 p-2.5 sm:p-3.5">
            <div className="mb-1.5 text-xs font-bold uppercase tracking-wider text-white/50">
              {localizeUi("ui.game.dndcombatui.dDCombatFeed")}
            </div>
      ...
      <div className="border-t border-white/10 bg-slate-900/98 p-2.5 sm:p-4 shrink-0 shadow-2xl">
        ...
        <div className="mb-2 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-1 overflow-x-auto no-scrollbar py-0.5">
            ...
            >
              {localizeUi("ui.game.dndcombatui.adv")}
            </button>
            <button
              type="button"
            ...
            >
              {localizeUi("ui.game.dndcombatui.disadv")}
            </button>
            {isRogue && (
              <button
                type="button"
              ...
              >
                {localizeUi("ui.game.dndcombatui.sneak")}
                {sneakDice}
                {localizeUi("ui.game.dndcombatui.d6")}
              </button>
            )}
            {isPaladin && (
              <button
              ...
              >
                {localizeUi("ui.game.dndcombatui.smite3d8")}
              </button>
            )}
          </div>
          ...
          <div className="flex items-center justify-between sm:justify-end gap-2 overflow-x-auto no-scrollbar">
            <span className="text-[0.6875rem] sm:text-xs font-bold text-amber-300 shrink-0">
              +{spellAttackBonus} {localizeUi("ui.game.dndcombatui.toHitDc")} {spellDc}
            </span>
            ...
            <div className="flex items-center gap-1 rounded-lg bg-black/50 p-0.5 border border-white/10 shrink-0">
              ...
              >
                {localizeUi("ui.game.dndcombatui.cantrips")}
              </button>
              <button
                type="button"
              ...
              >
                {localizeUi("ui.game.dndcombatui.lvl13")}
              </button>
              <button
                type="button"
              ...
              >
                {localizeUi("ui.game.dndcombatui.lvl4")}
              </button>
              <button
                type="button"
              ...
              >
                {localizeUi("ui.agents.agentcatalogview.allModes")}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-1.5 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 max-h-48 sm:max-h-40 overflow-y-auto overscroll-contain touch-pan-y pr-1 pb-1 scrollbar-thin scrollbar-thumb-white/20">
          ...
          <button
            ...
            title={localizeUi("ui.game.dndcombatui.value1Value2ToHitValue3", {
              value1: activeWeapon.name,
              value2: weaponAttackBonus,
              value3: activeWeapon.description,
            })}
          >
            ...
            <span className="text-[0.5625rem] sm:text-[0.625rem] text-amber-300/80 leading-none">
              {martialAttacks > 1 ? localizeUi("ui.game.dndcombatui.value1", { value1: martialAttacks }) : ""}
              {activeWeapon.damageDie}+{formatModifier(weaponMod)}{" "}
              {activeWeapon.mastery
                ? localizeUi("ui.game.dndcombatui.value1_aa52a84", { value1: activeWeapon.mastery })
                : ""}
            </span>
          </button>
          ...
          {visibleSpells.map((spell) => {
            ...
            const diceScaling = isCantrip
              ? `${cantripScaling(actorLevel)}${spell.damageDie || "d10"}`
              : `${spell.diceCount || 8}${spell.damageDie || "d6"}`;
            const isAoE = Boolean(
              spell.isAoE ||
              spell.id === "fireball" ||
              spell.id === "synaptic_static" ||
            ...
            return (
              ...
              >
                ...
                <span className="text-[0.5625rem] sm:text-[0.625rem] text-cyan-300/70 leading-none">
                  {spell.type === "save"
                    ? localizeUi("ui.game.dndcombatui.dcValue1Value2Save", {
                        value1: spellDc,
                        value2: spell.saveStat?.toUpperCase(),
                      })
                    : diceScaling}
                </span>
              </button>
          ...
          >
            <Shield size={15} />
            <span className="text-[0.6875rem] sm:text-xs">{localizeUi("ui.game.dndcombatui.dodgeEvade")}</span>
          </button>
          ...
          >
            ...
            <span className="text-[0.6875rem] sm:text-xs">{localizeUi("ui.game.tacticalcombatui.endTurn")}</span>
          </button>
        </div>
      </div>
          ...
          <div className="w-full max-w-lg rounded-2xl border border-cyan-500/40 bg-slate-900 p-4 sm:p-6 shadow-2xl">
            <div className="mb-3 sm:mb-4 flex items-center justify-between border-b border-white/10 pb-2 sm:pb-3">
              <h3 className="text-base sm:text-lg font-bold text-cyan-200">
                {localizeUi("ui.game.dndcombatui.editDDStats")} {editingCombatant.name}
              </h3>
              <button
                type="button"
            ...
            <div className="space-y-3 sm:space-y-4 max-h-[75vh] overflow-y-auto pr-1 overscroll-contain">
              ...
              <div className="rounded-xl bg-black/40 p-2.5 sm:p-3">
                <div className="mb-2 flex items-center justify-between">
                  <div className="flex items-center gap-1.5 text-xs sm:text-sm font-bold text-white">
                    ...
                    <span>{localizeUi("ui.game.dndcombatui.dDClass")}</span>
                  </div>
                  <span className="text-[0.6875rem] sm:text-xs font-semibold text-cyan-300">
                    {localizeUi("ui.game.dndcombatui.casting")}{" "}
                    {primaryCastingStat(editingCombatant.unitClass).toUpperCase()}
                  </span>
                </div>
                ...
                <input
                  ...
                  placeholder={localizeUi("ui.game.dndcombatui.eGWarlockWizardRogue")}
              ...
              <div className="rounded-xl bg-black/40 p-2.5 sm:p-3">
                <div className="mb-1.5 flex items-center justify-between">
                  <div className="flex items-center gap-1.5 text-xs sm:text-sm font-bold text-white">
                    ...
                    <span>{localizeUi("ui.game.dndcombatui.equippedWeapon")}</span>
                  </div>
                ...
                >
                  <option value="">
                    {localizeUi("ui.game.dndcombatui.autoClassDefault")}{" "}
                    {getCombatantWeapon({ ...editingCombatant, weaponId: undefined }).name})
                  </option>
                  {WEAPON_CATALOG.map((w) => (
                    <option key={w.id} value={w.id}>
                      {w.icon} {w.name} ({w.damageDie} {w.damageType}
                      {w.mastery ? localizeUi("ui.game.dndcombatui.masteryValue1", { value1: w.mastery }) : ""})
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div className="flex items-center justify-between rounded-xl bg-black/40 p-2.5 sm:p-3">
                <div>
                  <div className="text-xs sm:text-sm font-bold text-white">
                    {localizeUi("ui.game.dndcombatui.characterLevel")}
                  </div>
                  <div className="text-[0.6875rem] text-white/50">
                    {localizeUi("ui.game.dndcombatui.proficiency")}
                    {proficiencyBonus(editingCombatant.level)} {localizeUi("ui.game.dndcombatui.cantrips_011d51e")}{" "}
                    {cantripScaling(editingCombatant.level)}×
                  </div>
                </div>
                <div className="flex items-center gap-1.5">
                  <button
                  ...
                  <span className="w-6 text-center text-base sm:text-lg font-bold text-cyan-300">
                    {editingCombatant.level}
                  </span>
                  <button
                    type="button"
                    onClick={() => updateCombatantStat(editingCombatant.id, "level", editingCombatant.level + 1)}
              ...
              <div className="grid grid-cols-2 gap-2 sm:gap-3">
                <div className="rounded-xl bg-black/40 p-2.5 sm:p-3">
                  <div className="text-[0.6875rem] font-bold uppercase text-white/60">
                    {localizeUi("ui.game.dndcombatui.armorClassAc")}
                  </div>
                  <input
                    type="number"
                    min={1}
                ...
                <div className="rounded-xl bg-black/40 p-2.5 sm:p-3">
                  <div className="text-[0.6875rem] font-bold uppercase text-white/60">
                    {localizeUi("ui.game.dndcombatui.maxHp")}
                  </div>
                  <input
                    type="number"
                    min={1}
              ...
              >
                {localizeUi("ui.game.dndcombatui.saveReturnToBattle")}
              </button>
            </div>
          </div>
        </div>
      ...
      {fleeConfirm && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/80 p-4">
          <div className="w-full max-w-xs rounded-2xl border border-white/15 bg-slate-900 p-5 text-center shadow-2xl">
            ...
            <h4 className="mb-2 text-base font-bold text-white">{localizeUi("ui.game.dndcombatui.fleeFromBattle")}</h4>
            <p className="mb-4 text-xs text-white/70">
              {localizeUi("ui.game.dndcombatui.yourPartyWillDisengageAndRetreatBackToExploration")}
            </p>
            <div className="flex gap-2">
              ...
              >
                {localizeUi("ui.game.dndcombatui.stayFight")}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="absolute inset-0 z-50 flex flex-col items-center justify-center gap-4 bg-black/85 p-6 text-center backdrop-blur">
          {dndState.outcome === "victory" && (
            <>
              ...
              <h2 className="text-3xl font-black uppercase tracking-widest text-amber-300">
                {localizeUi("ui.game.dndcombatui.victory")}
              </h2>
              <p className="max-w-md text-sm text-white/80">
                {localizeUi("ui.game.dndcombatui.allEnemiesHaveBeenDefeatedReturnToTheNarrative")}
              </p>
            </>
          )}
          {dndState.outcome === "defeat" && (
            <>
              ...
              <h2 className="text-3xl font-black uppercase tracking-widest text-red-400">
                {localizeUi("ui.game.dndcombatui.defeat")}
              </h2>
              <p className="max-w-md text-sm text-white/80">
                {localizeUi("ui.game.dndcombatui.yourPartyWasOverwhelmedReturnToTheGmTo")}
              </p>
            </>
          )}
          {dndState.outcome === "fled" && (
            <>
              ...
              <h2 className="text-3xl font-black uppercase tracking-widest text-cyan-300">
                {localizeUi("ui.game.dndcombatui.retreat")}
              </h2>
              <p className="max-w-md text-sm text-white/80">
                {localizeUi("ui.game.dndcombatui.youSuccessfullyDisengagedAndFledFromCombat")}
              </p>
            </>
          )}
          ...
          >
            {localizeUi("ui.game.dndcombatui.continueStory")}
          </button>
        </div>
      )}
    </div>
