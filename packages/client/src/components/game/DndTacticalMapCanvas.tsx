...
import { Shield, Flame, Footprints, Crown, ZoomIn, ZoomOut, Maximize2, FastForward, Sword } from "lucide-react";
...
export function DndTacticalMapCanvas({
  ...
  const handleTileClick = (tile: DndTile) => {
    ...
    if (activeUnit && activeUnitState) {
      ...
      const oppAttack = checkOpportunityAttack(activeUnit.id, activeUnitState.coord, targetCoord, livingEnemies, units);
      ...
      <div className="flex items-center justify-between border-b border-white/10 bg-black/50 px-3 py-2 text-xs">
        <div className="flex items-center gap-3">
          <span className="font-bold text-cyan-300">
            {localizeUi("ui.game.dndtacticalmapcanvas.dDBattlemap5ftGrid")}
          </span>
          ...
          {isAnimating ? (
            <div className="flex items-center gap-2 rounded-lg bg-amber-500/20 px-2.5 py-1 text-xs font-bold text-amber-300 border border-amber-500/40 animate-pulse">
              ...
              {onSkipAnimation && (
                ...
                >
                  <FastForward size={12} />
                  <span>{localizeUi("onboarding.actions.skip")}</span>
                </button>
              )}
            </div>
          ) : (
            <>
                ...
                <span className="flex items-center gap-1 font-semibold text-white/80">
                  ...
                  <span>
                    ...
                    <strong className="text-cyan-300">
                      {activeUnitState.movementRemainingFt} {localizeUi("ui.game.dndtacticalmapcanvas.ft")}
                    </strong>{" "}
              ...
              {activeAoESpell && (
                <span className="flex items-center gap-1 rounded bg-orange-500/20 px-2 py-0.5 font-bold text-orange-300 animate-pulse border border-orange-500/40">
                  <Flame size={14} />
                  <span>
                    {activeAoESpell.aoeShape === "cone" ? "📐" : activeAoESpell.aoeShape === "line" ? "⚡" : "💥"}{" "}
                    {localizeUi("ui.game.dndtacticalmapcanvas.tapToCast")} {activeAoESpell.name} (
                    {activeAoESpell.aoeRadiusFt || 20}
                    {localizeUi("ui.game.dndtacticalmapcanvas.ft")} {activeAoESpell.aoeShape || "sphere"})
                  </span>
                </span>
              )}
            </>
        ...
        <div className="flex items-center gap-1.5">
          <button
            ...
            title={localizeUi("ui.game.dndtacticalmapcanvas.zoomOut")}
          >
            <ZoomOut size={14} />
          </button>
          <span className="w-9 text-center font-mono text-[0.6875rem] text-white/60">
          ...
          <button
            ...
            className="rounded bg-white/10 p-1 text-white/70 hover:bg-white/20 hover:text-white"
            title={localizeUi("ui.game.dndtacticalmapcanvas.zoomIn")}
          >
            <ZoomIn size={14} />
          </button>
          <button
            ...
            className="rounded bg-white/10 p-1 text-white/70 hover:bg-white/20 hover:text-white"
            title={localizeUi("ui.game.dndtacticalmapcanvas.resetZoom")}
          >
            <Maximize2 size={14} />
          </button>
        </div>
                ...
                return (
                  <div
                    ...
                    className={cn(
                      ...
                      isReachable &&
                        "bg-cyan-500/20 border-cyan-400/80 shadow-lg shadow-cyan-900/40 ring-1 ring-cyan-400/50",
                      isAoEHit &&
                        "bg-orange-600/40 border-orange-400 shadow-lg shadow-orange-900/50 ring-2 ring-orange-400",
                  ...
                  >
                    ...
                    {isTargetFlash && (
                      <div className="absolute inset-0 z-30 flex items-center justify-center bg-red-600/40 rounded-lg animate-pulse">
                        <span className="text-sm font-black text-white drop-shadow">
                          {localizeUi("ui.game.dndtacticalmapcanvas.hit")}
                        </span>
                      </div>
                    )}
                    ...
                    {tile.coverLevel !== "none" && !combatant && (
                      <span className="absolute bottom-0.5 right-1 flex items-center gap-0.5 text-[0.5625rem] font-bold text-stone-400">
                        <Shield size={10} />
                        <span>{localizeUi("ui.game.dndtacticalmapcanvas.text2Ac")}</span>
                      </span>
                    )}

                    {/* Reachable Move Footprint Indicator */}
                    {isReachable && !combatant && <Footprints size={16} className="text-cyan-300/60 animate-pulse" />}
                      ...
                      <div className="relative flex flex-col items-center justify-center z-10">
                        <div
                          className={cn(
                            ...
                            isTurnActor &&
                              "scale-125 ring-4 ring-yellow-300 bg-amber-900 text-yellow-100 shadow-2xl shadow-yellow-400/90 animate-bounce",
                          )}
                        >
                          {combatant.isPlayer && (
      ...
      <div className="flex flex-wrap items-center justify-between border-t border-white/10 bg-black/60 px-3 py-1.5 text-[0.6875rem] text-white/60">
        <div className="flex items-center gap-3">
          <span className="flex items-center gap-1">
            ...
            {localizeUi("ui.game.dndtacticalmapcanvas.partyAlly")}
          </span>
          <span className="flex items-center gap-1">
            <span className="h-2.5 w-2.5 rounded-full bg-amber-400" />{" "}
            {localizeUi("ui.game.dndtacticalmapcanvas.protagonistYou")}
          </span>
          <span className="flex items-center gap-1">
            <span className="h-2.5 w-2.5 rounded-full bg-red-500" /> {localizeUi("ui.game.dndtacticalmapcanvas.enemy")}
          </span>
          <span className="flex items-center gap-1">
            <span className="h-2.5 w-2.5 rounded bg-cyan-500/30 border border-cyan-400" />{" "}
            {localizeUi("ui.game.dndtacticalmapcanvas.moveRange5ftTile")}
          </span>
          <span className="flex items-center gap-1">
            <span className="h-2.5 w-2.5 rounded bg-orange-600/40 border border-orange-400" />{" "}
            {localizeUi("ui.game.dndtacticalmapcanvas.aoeBlastZone")}
          </span>
        </div>

        <div>
          <span>{localizeUi("ui.game.dndtacticalmapcanvas.tapEmptyBlueTileToMoveTapEnemyTo")}</span>
        </div>
      </div>
    </div>
  );
